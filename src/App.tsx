  ...
  return (
    <div className="min-h-screen bg-brand-dark flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* ── HEADER ──
            Mobile:  stacks vertically — toggle on top, search below
            Desktop: single row — toggle + search side by side           */}
        ...
        <main className="flex-1 bg-brand-dark p-4 md:p-12 overflow-y-auto">
          ...
          {activeSection === 'desktop' && (
            <>
              <section className="mb-12 md:mb-20">
                <motion.h2 key="dh" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="text-6xl md:text-[10rem] font-black italic leading-none tracking-tight">
                  {selectedCategory === 'Favorites' ? 'SAVED' : 'VELO'}
                  <span className="text-brand-line">{selectedCategory === 'Favorites' ? 'CARS' : 'CITY'}</span>
                </motion.h2>
                <div className="flex items-center gap-4 mt-4 md:mt-6">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/40">
                    {selectedCategory === 'Favorites' ? 'YOUR PERSONAL COLLECTION' : selectedBrand ? `FILTERING — ${selectedBrand.toUpperCase()}` : 'PREMIUM AUTOMOTIVE REPOSITORY'}
                  </p>
                </div>
              </section>
              ...
              <div className="flex flex-wrap gap-2 mb-4 md:mb-6">
                <button onClick={() => setSelectedCategory('All')} className={filterBtn(selectedCategory === 'All')}>All</button>
                <div className="w-px h-8 bg-brand-line mx-1" />
                {CATEGORIES.filter(c => c !== 'All').map(cat => (
                  <button key={cat} onClick={() => setSelectedCategory(cat)} className={filterBtn(selectedCategory === cat)}>{cat}</button>
                ))}
              </div>

              {/* Brand filter row */}
              <div className="flex flex-wrap gap-2 mb-8 md:mb-12 border-b border-brand-line pb-6 md:pb-8">
                <button onClick={() => setSelectedBrand(null)} className={filterBtn(!selectedBrand)}>All Brands</button>
                <div className="w-px h-8 bg-brand-line mx-1" />
                {brands.map(([brand]) => (
                  <button key={brand} onClick={() => setSelectedBrand(prev => prev === brand ? null : brand)} className={filterBtn(selectedBrand === brand)}>{brand}</button>
                ))}
              </div>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredWallpapers.map(car => (
                    <Link key={car.id} to={`/brand/${car.brand.toLowerCase()}/${car.slug}`}>
                      <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="group relative cursor-pointer border border-brand-line bg-brand-medium overflow-hidden">
                        <div className="relative aspect-[4/3] overflow-hidden">
                          <motion.img src={car.imageUrl} alt={`${car.brand} ${car.title} wallpaper`} loading="lazy" decoding="async" sizes="(max-width: 768px) 100vw, 50vw" className="w-full h-full object-cover transition-all duration-700" />
                          <button
                            onClick={e => toggleFavorite(e, car.id)}
                            className={`absolute top-4 right-4 md:top-6 md:right-6 z-10 p-2.5 md:p-3 glass backdrop-blur-md transition-all duration-300 ${favorites.includes(car.id) ? 'bg-red-500 border-red-500 opacity-100' : 'opacity-0 group-hover:opacity-100 bg-white/10 hover:bg-white/20'}`}
                          >
          ...
          {/* ════════════════════════════════════════
              ...
              — identical layout/brightness to desktop
          ════════════════════════════════════════ */}
          {activeSection === 'phone' && (
            <>
              <section className="mb-12 md:mb-20">
                ...
                <div className="flex items-center gap-4 mt-4 md:mt-6">
                  <div className="h-[1px] bg-brand-line flex-1" />
                  <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/40">
                    {selectedPhoneBrand ? `FILTERING — ${selectedPhoneBrand.toUpperCase()}` : `${filteredPhoneWallpapers.length} MOBILE WALLPAPERS`}
                  </p>
                </div>
              </section>

              {/* Phone brand filter row — only shown if there are named brands */}
              {phoneBrands.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-8 md:mb-12 border-b border-brand-line pb-6 md:pb-8">
                  <button onClick={() => setSelectedPhoneBrand(null)} className={filterBtn(!selectedPhoneBrand)}>All</button>
                  <div className="w-px h-8 bg-brand-line mx-1" />
                  {phoneBrands.map(([brand]) => (
                    <button key={brand} onClick={() => setSelectedPhoneBrand(prev => prev === brand ? null : brand)} className={filterBtn(selectedPhoneBrand === brand)}>{brand}</button>
                  ))}
                </div>
              )}

              {/* Phone grid — same card style as desktop */}
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                {filteredPhoneWallpapers.map(wallpaper => (
                  <Link key={wallpaper.slug} to={`/phone/${wallpaper.slug}`} className="group">
                    {/* Card matches desktop: border + bg */}
                    <div className="border border-brand-line bg-brand-medium overflow-hidden">
                      ...
                      <div className="relative aspect-[9/19] overflow-hidden">
                        ...
                        />
                        {/* Dynamic island */}
                        <div className="absolute top-2.5 left-1/2 -translate-x-1/2 w-14 h-4 bg-black/80 rounded-full z-10 pointer-events-none" />
                        {/* Hover overlay */}
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all duration-300 pointer-events-none" />
                        <div className="absolute inset-0 bg-linear-to-t from-black via-transparent to-transparent opacity-50 pointer-events-none" />
                        {/* Title overlay on hover — same as desktop cards */}
                        <div className="absolute inset-x-0 bottom-0 p-3 md:p-4 translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
                          <h3 className="text-sm md:text-base font-display font-black italic uppercase tracking-tighter leading-none">{wallpaper.title}</h3>
                          {wallpaper.brand && wallpaper.brand !== 'Mobile' && (
                            <p className="text-[9px] font-bold uppercase tracking-[0.3em] text-white/40 mt-0.5">{wallpaper.brand}</p>
                          )}
                        </div>
                      </div>
                    </div>
              ...
              {filteredPhoneWallpapers.length === 0 && (
                <div className="py-32 text-center border-2 border-dashed border-brand-line">
                  <h3 className="text-xl font-display font-black uppercase tracking-wider text-white/20 italic">NOTHING FOUND IN THE GRID</h3>
                </div>
              )}
            </>
          )}
      ...
      <aside className="w-80 hidden xl:flex flex-col p-12 border-l border-brand-line bg-brand-surface shrink-0">
        <div className="flex flex-col gap-3 mb-12">
          <button
            ...
            className={`w-full flex items-center justify-between px-5 py-4 text-[10px] font-black uppercase tracking-widest transition-all border ${selectedCategory === 'Favorites' ? 'bg-white text-black border-white' : 'bg-transparent text-white/50 border-brand-line hover:border-white/40 hover:text-white'}`}
          >
          ...
          {user ? (
            <div className="border border-brand-line p-4 flex items-center justify-between">
              ...
              <button onClick={handleLogout} className="text-white/40 hover:text-white transition"><LogOut className="w-4 h-4" /></button>
            </div>
          ) : (
            <button onClick={handleLogin} disabled={isLoggingIn} className="w-full flex items-center justify-center gap-3 bg-white text-black px-8 py-4 text-[10px] font-black uppercase tracking-widest hover:bg-white/90 transition-all disabled:opacity-50">
      ...
      <AnimatePresence>
        {selectedWallpaper && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div layoutId={selectedWallpaper.id} className="relative w-full max-w-5xl bg-brand-medium rounded-none border border-white/10 flex flex-col my-auto">
              <button onClick={() => setSelectedWallpaper(null)} className="absolute top-4 right-4 md:top-6 md:right-6 z-20 p-2 bg-black hover:bg-white hover:text-black transition-all">
                <X className="w-5 h-5 md:w-6 md:h-6" />
              </button>
              <div className="flex flex-col md:flex-row">
                <div className="flex-1 min-h-0 bg-black flex items-center justify-center relative" style={{ minHeight: '260px' }}>
                  <img src={selectedWallpaper.imageUrl} alt={selectedWallpaper.title} className="w-full h-full object-contain" />
                  <button onClick={e => toggleFavorite(e, selectedWallpaper.id)}
                ...
                <div className="w-full md:w-80 p-6 md:p-10 flex flex-col justify-start border-t md:border-t-0 md:border-l border-white/10">
                  <div>
                    ...
                    <h2 className="text-3xl md:text-4xl font-display font-black italic uppercase tracking-tighter mb-2 leading-[0.9]">{selectedWallpaper.title}</h2>
                    <p className="text-white/40 text-[10px] font-black uppercase tracking-[0.4em] mb-8 md:mb-12">{selectedWallpaper.brand}</p>
                  </div>
                  <div className="mt-2 space-y-3">
                    <button onClick={() => handleDownload(selectedWallpaper)} disabled={isDownloading}
                    ...
                    <button onClick={() => setSelectedWallpaper(null)} disabled={isDownloading}
              ...
              <div className="border-t border-white/10">
                <div className="flex border-b border-white/10 overflow-x-auto">
                  <button onClick={() => setActiveTab('info')} className={`flex items-center gap-2 px-5 md:px-8 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b-2 whitespace-nowrap ${activeTab === 'info' ? 'border-white text-white' : 'border-transparent text-white/30 hover:text-white/60'}`}>
                    <BookOpen className="w-3.5 h-3.5" />ABOUT & HISTORY
                  </button>
                  <button onClick={() => setActiveTab('similar')} className={`flex items-center gap-2 px-5 md:px-8 py-4 text-[10px] font-black uppercase tracking-widest transition-all border-b-2 whitespace-nowrap ${activeTab === 'similar' ? 'border-white text-white' : 'border-transparent text-white/30 hover:text-white/60'}`}>
                    <Sparkles className="w-3.5 h-3.5" />YOU MAY ALSO LIKE
                  </button>
                </div>
                <AnimatePresence mode="wait">
                  {activeTab === 'info' && (
                    <motion.div key="info" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="p-5 md:p-8 min-h-[140px]">
                      {carInfoLoading ? (
                        <div className="flex flex-col gap-3">
                          <div className="flex items-center gap-3 text-white/30 mb-2">
                            <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1.2, ease: 'linear' }}><Gauge className="w-4 h-4" /></motion.div>
                            <span className="text-[10px] font-black uppercase tracking-widest">LOADING...</span>
                          </div>
                          {[75, 55, 85, 45, 65, 50].map((w, i) => (
                            <motion.div key={i} className="h-2.5 bg-white/5 rounded-sm" style={{ width: `${w}%` }} animate={{ opacity: [0.3, 0.7, 0.3] }} transition={{ repeat: Infinity, duration: 1.5, delay: i * 0.12 }} />
                          ))}
                        </div>
                      ) : carInfo ? (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                          <div><p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-3">ABOUT</p><p className="text-sm text-white/70 leading-relaxed font-medium">{carInfo.about}</p></div>
                          <div><p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-3">HERITAGE</p><p className="text-sm text-white/70 leading-relaxed font-medium">{carInfo.history}</p></div>
                          <div><p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/30 mb-3">DETAILS</p>
                            <ul className="space-y-2">{carInfo.facts.map((fact, i) => (<li key={i} className="flex items-start gap-2"><span className="text-white/20 font-black text-[10px] mt-0.5 shrink-0">✦</span><span className="text-[11px] text-white/60 font-medium leading-snug">{fact}</span></li>))}</ul>
                          </div>
                        </div>
                      ) : null}
                    </motion.div>
                  ...
                  {activeTab === 'similar' && (
                    <motion.div key="similar" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="p-5 md:p-8">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                        {similarCars.map(car => (
                          <Link key={car.id} to={`/brand/${car.brand.toLowerCase()}/${car.slug}`} onClick={() => setSelectedWallpaper(null)} className="group border border-white/10 hover:border-white/30 transition-all overflow-hidden bg-black">
                            <div className="relative aspect-[4/3] overflow-hidden">
                              <img src={car.imageUrl} alt={car.title} className="w-full h-full object-cover brightness-50 group-hover:brightness-90 transition-all duration-500" />
                            ...
                            <div className="p-1.5 md:p-2 flex gap-1">
                              <button onClick={e => { e.preventDefault(); e.stopPropagation(); handleDownload(car); }} className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-white/5 hover:bg-white hover:text-black text-white/40 text-[8px] font-black uppercase tracking-widest transition-all">
                                <Download className="w-3 h-3" /><span className="hidden sm:inline">DOWNLOAD</span>
                              </button>
                              <button onClick={e => { e.preventDefault(); toggleFavorite(e, car.id); }} className={`p-2 transition-all ${favorites.includes(car.id) ? 'bg-red-500 text-white' : 'bg-white/5 hover:bg-white/10 text-white/40'}`}>
