      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] min-h-screen lg:h-screen border-b border-zinc-800 lg:overflow-hidden">
        ...
        <div className="relative overflow-hidden bg-black min-h-[55vh] lg:min-h-0">
          <Link to="/" className="absolute top-8 left-8 z-50 group">
            <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border border-white/20 bg-black/60 backdrop-blur-xl flex items-center justify-center transition-all duration-300 group-hover:bg-white group-hover:border-white shadow-2xl">
        ...
        <div className="bg-[#050505] border-t lg:border-t-0 lg:border-l border-zinc-800 flex flex-col lg:h-screen overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          <div className="flex-1 p-8 md:p-10">
            ...
            <span className="inline-block border border-white px-3 py-1 text-[10px] font-bold tracking-[0.25em] uppercase mb-5 text-white bg-zinc-950">
              {car.category}
            </span>
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-black italic uppercase leading-[0.9] tracking-tight mb-2 text-white">
              {car.title}
            </h1>
            <p className="text-[11px] uppercase tracking-[0.35em] text-zinc-300 mb-6">
              {car.brand}
            </p>
            ...
            <div className="border-t border-zinc-800 pt-4">
              <p className="text-[9px] font-black uppercase tracking-[0.3em] text-zinc-400 mb-3">DETAILS</p>
              <ul className="space-y-2">
                {[`Category: ${car.category}`, `Brand: ${car.brand}`, 'Premium wallpaper collection', '4K Ultra HD resolution'].map((fact) => (
                  <li key={fact} className="flex items-start gap-2">
                    <span className="text-zinc-400 font-black text-[10px] mt-0.5 shrink-0">✦</span>
                    <span className="text-[11px] text-zinc-300 font-medium leading-snug">{fact}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {relatedWallpapers.map((w) => (
            <Link key={w.id} to={`/brand/${brandToUrl(w.brand)}/${w.slug}`} className="group border border-zinc-900 bg-zinc-950 overflow-hidden hover:border-white/30 transition-all duration-300">
              ...
              <div className="p-4">
                <p className="text-[10px] uppercase tracking-[0.3em] text-zinc-400 mb-1">{w.brand}</p>
                <h3 className="text-base font-black italic uppercase leading-tight text-white">{w.title}</h3>
              </div>
            </Link>
          ))}
        </div>
